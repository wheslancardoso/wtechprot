...
import { signCustodyTerm } from '../actions' // Import Server Action
...
export default function CheckinPage({ params }: CheckinPageProps) {
    ...
    const validateStep = () => {
        ...
        if (step === 'signature') {
            if (!geolocation) {
                toast({ title: 'Localização Obrigatória', description: 'Clique em "Capturar Localização" antes de assinar.', variant: 'destructive' })
                return false
            }
            if (!signatureUrl) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            // Submit to Server Action (Hybrid Signature)
            const result = await signCustodyTerm(targetUuid, {
                accessories,
                conditions,
    ...
    const getStepTitle = () => {
        switch (step) {
            ...
            case 'signature': return 'Assinatura Híbrida'
    ...
    return (
        <div className="min-h-screen bg-background pb-24 font-sans text-foreground">
            ...
            <main className="p-4 max-w-lg mx-auto space-y-6 mt-4">
                ...
                {step === 'signature' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="bg-card border p-4 sm:p-5 rounded-xl text-xs text-muted-foreground leading-relaxed font-mono shadow-sm h-48 overflow-y-auto">
                            <h3 className="font-bold mb-3 text-sm text-foreground uppercase">Termo de Custódia e Retirada:</h3>
                            <p className="mb-2">Eu, abaixo assinado, na qualidade de proprietário ou reponsável pelo equipamento:</p>
                            <ul className="space-y-2 list-disc pl-4 mb-3">
                                <li>Declaro entregar o equipamento descrito para análise técnica e orçamento.</li>
                                <li>Confirmo que a lista de acessórios e as condições físicas reportadas (fotos e descrição) conferem com a realidade.</li>
                                <li>Autorizo o transporte do equipamento para as dependências da assistência.</li>
                                <li>Isento a empresa de responsabilidade por perda de dados não backupeados.</li>
                            </ul>
                        </div>

                        {/* Location Step */}
                        <div className="p-4 bg-muted/30 rounded-xl border border-dashed flex flex-col items-center gap-3 text-center">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground max-w-xs">
                                    {geolocation
                                        ...
                                        : "Para segurança jurídica, precisamos registrar onde o equipamento foi retirado."
                                    }
                                </p>
                            </div>
                            <Button
                                ...
                                disabled={loading}
                                className={cn("w-full sm:w-auto", geolocation && "border-green-500 text-green-600 hover:text-green-700")}
                            >
                        ...
                        <div className="space-y-2">
                            <Label className="text-sm font-semibold block mb-2">Assinatura do Responsável</Label>
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <Card className="border-border/60 shadow-lg">
                            <CardContent className="pt-6 space-y-6 text-sm">
                                ...
                                <div>
                                    <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider block mb-3">Assinatura Híbrida</span>
                                    <div className="bg-muted/30 p-3 rounded-lg border border-dashed mb-3">
                                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                            <MapPin className="h-3 w-3" />
                                            <span>Local: {geolocation?.lat.toFixed(5)}, {geolocation?.lng.toFixed(5)}</span>
                                        </div>
                                    </div>
                                    {signatureUrl && (
